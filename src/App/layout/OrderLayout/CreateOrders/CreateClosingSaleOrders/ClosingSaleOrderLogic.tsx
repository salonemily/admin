...
import SaleOrderView from "./ClosingSaleOrderView";
import { v4 as uuid } from "uuid";
import { Segment } from "semantic-ui-react";
import LoaderComponent from "../../../../utilities/LoaderComponent";
import { DressFormValues } from "../../../../models/salonDress";
import { ClosingSaleOrderFormValues } from "../../../../models/closingSaleOrder";
import { ClosingSaleOrderSummaryData, submitStateInit } from "../common/ordersAddons";
import { PostClosingSaleOrder } from "../../../../stateManagment/action/dataManagment/ClosingSaleOrder/ClosingSaleOrderPostAction";

const CreateClosingSaleOrder = () => {
  ...
  const isSuccesfull = useSelector(
    (state: RootStore) => state.postClosingSaleOrderReducer.isSuccesfull
  );
  const isLoading = useSelector(
    (state: RootStore) => state.postClosingSaleOrderReducer.isLoading
  );
  ...
  const [data, setData] = useState(new ClosingSaleOrderSummaryData());
  ...
  const handleOrderSubmit = (values: ISaleOrderPost) => {
   ...
   if(isClientReady && isDressReady) setTabState({ activeIndex: 3 });
  ...
  const handleMainSubmit = () => {
    ...
    if (dataOrder && dataDress && dataClient) {
      ...
      dispatch(PostClosingSaleOrder(orderwithID, dataClient, dataDress, clienttype,dresstype));
    }
  };
...
export default CreateClosingSaleOrder;
